...
# Creates the bodies component table.
#
...
# (RFC-0006).
class CreateBodies < ActiveRecord::Migration[8.1]
  def change
    create_table :bodies, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :bodies, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :bodies, :entities, column: :entity_id, on_delete: :cascade
  end
end
