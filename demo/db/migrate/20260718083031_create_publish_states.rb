...
# Creates the publish_states component table.
#
...
# (RFC-0006).
class CreatePublishStates < ActiveRecord::Migration[8.1]
  def change
    create_table :publish_states, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :publish_states, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :publish_states, :entities, column: :entity_id, on_delete: :cascade
  end
end
