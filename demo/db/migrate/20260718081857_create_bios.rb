...
# Creates the bios component table.
#
...
class CreateBios < ActiveRecord::Migration[8.1]
  def change
    create_table :bios, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :bios, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :bios, :entities, column: :entity_id, on_delete: :cascade
  end
end
