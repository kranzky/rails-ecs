...
# Creates the titles component table.
#
...
class CreateTitles < ActiveRecord::Migration[8.1]
  def change
    create_table :titles, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :titles, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :titles, :entities, column: :entity_id, on_delete: :cascade
  end
end
