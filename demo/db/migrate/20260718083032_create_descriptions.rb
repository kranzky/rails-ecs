...
# Creates the descriptions component table.
#
...
class CreateDescriptions < ActiveRecord::Migration[8.1]
  def change
    create_table :descriptions, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :descriptions, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :descriptions, :entities, column: :entity_id, on_delete: :cascade
  end
end
