...
# Creates the member_users component table.
#
...
class CreateMemberUsers < ActiveRecord::Migration[8.1]
  def change
    create_table :member_users, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :member_users, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :member_users, :entities, column: :entity_id, on_delete: :cascade
  end
end
