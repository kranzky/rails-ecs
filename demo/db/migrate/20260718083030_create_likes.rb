...
# Creates the likes component table.
#
...
class CreateLikes < ActiveRecord::Migration[8.1]
  def change
    create_table :likes, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
      ...
      t.integer :count, default: nil
      t.timestamps
    end

    # ADR-0005: a component appears at most once per entity.
    add_index :likes, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :likes, :entities, column: :entity_id, on_delete: :cascade
  end
end
