...
# Creates the administrators component table.
#
...
class CreateAdministrators < ActiveRecord::Migration[8.1]
  def change
    create_table :administrators, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
      t.uuid :entity_id, null: false
      t.timestamps
    end

    # ADR-0005: a component appears at most once per entity.
    add_index :administrators, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :administrators, :entities, column: :entity_id, on_delete: :cascade
  end
end
