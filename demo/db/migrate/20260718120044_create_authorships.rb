...
# Creates the authorships component table.
#
...
class CreateAuthorships < ActiveRecord::Migration[8.1]
  def change
    create_table :authorships, id: :uuid, default: -> { "gen_random_uuid()" } do |t|
    ...
    add_index :authorships, :entity_id, unique: true

    # Destroying an entity destroys its components, at the database level.
    add_foreign_key :authorships, :entities, column: :entity_id, on_delete: :cascade
  end
end
